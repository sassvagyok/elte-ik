...
sumFruits xs = sum [e | (FruitBatch _ e) <- xs]
...
sumDifferentFruits (FruitBatch Apple x : xs) = (x+a1, a2, a3) where
  (a1, a2, a3) = sumDifferentFruits xs
sumDifferentFruits (FruitBatch Grape x : xs) = (a1, x+a2, a3) where
  (a1, a2, a3) = sumDifferentFruits xs
sumDifferentFruits (FruitBatch Pear x : xs) = (a1, a2, x+a3) where
  (a1, a2, a3) = sumDifferentFruits xs
...
data TMP = TMP (Int -> Int)

instance Show TMP where
  show :: TMP -> String
  show (TMP f) = "A TMP-ben lévö f függvény értékei f(0)=" ++ show (f 0) ++ ", f(5)=" ++ show (f 5)
...
instance Num NumberOfApples where
  (+) (NumberOfApples a1) (NumberOfApples a2) = NumberOfApples (a1+a2)
  (*) (NumberOfApples a1) (NumberOfApples a2) = NumberOfApples (a1*a2)
  abs (NumberOfApples a1) = NumberOfApples (abs a1)
  signum (NumberOfApples a1) = NumberOfApples (signum a1)
  fromInteger i = NumberOfApples i
  negate (NumberOfApples a) = NumberOfApples (negate a)

instance Num NumberOfGrapes where
  (+) (NumberOfGrapes a1) (NumberOfGrapes a2) = NumberOfGrapes (a1+a2)
  (*) (NumberOfGrapes a1) (NumberOfGrapes a2) = NumberOfGrapes (a1*a2)
  abs (NumberOfGrapes a1) = NumberOfGrapes (abs a1)
  signum (NumberOfGrapes a1) = NumberOfGrapes (signum a1)
  fromInteger i = NumberOfGrapes i
  negate (NumberOfGrapes a) = NumberOfGrapes (negate a)

instance Num NumberOfPears where
  (+) (NumberOfPears a1) (NumberOfPears a2) = NumberOfPears (a1+a2)
  (*) (NumberOfPears a1) (NumberOfPears a2) = NumberOfPears (a1*a2)
  abs (NumberOfPears a1) = NumberOfPears (abs a1)
  signum (NumberOfPears a1) = NumberOfPears (signum a1)
  fromInteger i = NumberOfPears i
...
sumDifferentFruits' (FruitBatch Apple x : xs) = (NumberOfApples x + a, g, p) where
  (a, g, p) = sumDifferentFruits' xs
sumDifferentFruits' (FruitBatch Grape x : xs) = (a, NumberOfGrapes x + g, p) where
  (a, g, p) = sumDifferentFruits' xs
sumDifferentFruits' (FruitBatch Pear x : xs) = (a, g, NumberOfPears x + p) where
  (a, g, p) = sumDifferentFruits' xs
...
instance Num Complex where
  ...
  negate (Complex a1 b1) = Complex (-a1) (-b1)
  abs (Complex a1 b1) = Complex (sqrt (a1**2 + b1 ** 2)) 0
  signum (Complex a1 b1) = Complex (a1 / (sqrt (a1**2 + b1 ** 2))) (b1 / (sqrt (a1**2 + b1 ** 2)))
...
instance Fractional Complex where
  recip (Complex a1 b1) = Complex (a1/(a1**2+b1**2)) (-b1/(a1**2+b1**2))
