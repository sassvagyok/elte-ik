...
public class ÖnkormányzatTest
{
    ...
    public void Initialize()
    {
        ...
        {
            ok.SzervizelésIndítása(m.jármű, m.szerviz, m.javítás);
        }
        ...
        if (ok.szervizek.Any())
        {
            var sz = ok.szervizek.First();

            if (sz.munkalapok.Any())
            {
                var m = sz.munkalapok.First();
                sz.Lezár(m);
            }
        }
    ...
    [TestMethod]
    public void TestElöregedettArány()
    {
        Assert.AreEqual(0.5, ok.ElöregedettArány());
    }
    
    [TestMethod]
    public void TestJavításArány()
    {
        Assert.AreEqual(0.25, ok.JavításArány());
    }
    
    [TestMethod]
    public void TestLegdrágább()
    {
        Assert.IsNotNull(ok.Legdrágább());
    }
}